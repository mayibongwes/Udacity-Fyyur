...
def search_venues():
  ...
  try:
      ...
      data = []
      for venue in Venue.query.filter(Venue.name.contains(name)).all():
        data.append({
          "id": venue.id,
          "name": venue.name,
          "num_upcoming_shows": 0  #Mayi count upcoming shows
...
def show_venue(venue_id):
  ...
  if venue is not None:
    data = {
      "id": venue.id,
      "name":venue.name,
      "genres": ["Jazz", "Reggae", "Swing", "Classical", "Folk"], # Mayi add enum for venue genres
      "address": venue.address,
      "city": venue.city,
      "state": venue.state,
      "phone": venue.phone,
      "website": "https://www.themusicalhop.com",  # Mayi add website colum to venue table
      "facebook_link": venue.facebook_link,
      "seeking_talent": True, # Mayi add seeking talent to venue table
      "seeking_description": "We are on the lookout for a local artist to play every two weeks. Please call us.", # Mayi add Seeking description to venue table
